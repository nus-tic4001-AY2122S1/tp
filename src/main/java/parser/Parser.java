...
public class Parser {
    ...
    public Command parse(String input) throws ErrorHandler {
        ...
        switch (commandWord) {
            case LOCATION:
                if (result.length < 2) throw new ErrorHandler(ErrorMessage.EMPTY_TASK_NUMBER);
                 this.taskNo = result[1].trim();

                if (result.length < 3) throw new ErrorHandler(ErrorMessage.EMPTY_LOCATION_DESCRIPTION);
